...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    ...
    // gets first random pizza container
    var newwidth = (document.querySelector(".randomPizzaContainer").offsetWidth + dx) + 'px';
    // gets first random pizza container and gets its width.
    var elements = document.querySelectorAll(".randomPizzaContainer");
    //applies newwidth to all .randomPizzaContainer elements.
...
function updatePositions() {
  ...
  //used getElementsByClassName which is faster than querySelectorAll
  var items = document.getElementsByClassName('mover');
  //only needs to be calculated once when the function is called.
  var topOfPage = (document.body.scrollTop / 1250);
  var sameNums = [];

  // calculate the 5 numbers needed for phase.
  for(var i = 0;i < 5;i++){
